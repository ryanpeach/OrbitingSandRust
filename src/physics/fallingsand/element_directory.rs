...
/// Each "item" is split into sets that can be run in parallel spread out over 9 frames
/// So the way to process this would be to:
///   1. Run everything in standard_convolution[frame_nb % 9] in parallel
///   2. Run everything in the has_single_bottom_neighbor[frame_nb % 9] set in parallel
///   3. Run everything in the has_multi_bottom_neighbor[frame_nb % 9] set in parallel
///   4. increment frame_nb and run again next frame
#[derive(Clone)]
struct ProcessTargets {
    standard_convolution: [HashSet<ChunkIjkVector>; 9],
    has_single_bottom_neighbor: [HashSet<ChunkIjkVector>; 9],
...
/// 3x3 convolution kernels. It excludes known edge cases, like the bottom of a layer.
/// Run this over 0..9 frame_nb to get the targets for each frame
fn calculate_has_single_bottom_neighbor_targets(coords: CoordinateDir) -> HashSet<ChunkIjkVector> {
    let mut out = HashSet::new();
...
fn pregen_process_targets(coords: &CoordinateDir) -> ProcessTargets {
    let mut standard_convolution: [HashSet<ChunkIjkVector>; 9] = Default::default();
    for i in 0..9 {
        standard_convolution[i] = calculate_ith_standard_convolution_targets(coords.clone(), i);
    }
    let has_single_bottom_neighbor: [HashSet<ChunkIjkVector>; 9] = spread(
        calculate_has_single_bottom_neighbor_targets(coords.clone()),
        9,
    );
    ProcessTargets {
...
impl ElementGridDir {
    ...
    pub fn get_next_targets(&mut self) -> HashSet<ChunkIjkVector> {
        let out1 = self.process_targets.standard_convolution[self.process_count % 9].clone();
        let out2 = self.process_targets.has_single_bottom_neighbor[self.process_count % 9].clone();
        self.process_count += 1;
        out1.union(&out2).cloned().collect()
    }
        ...
        fn test_standard_convolution_packaging() {
            ...
            for frame_nb in 0..9 {
                let res = element_grid_dir
                    .package_convolutions(process_targets.standard_convolution[frame_nb].clone());
                match res {
                    Ok((convolutions, target_chunks)) => {
        ...
        fn test_has_single_bottom_neighbor_packaging() {
            ...
            for frame_nb in 0..9 {
                let res = element_grid_dir.package_convolutions(
                    process_targets.has_single_bottom_neighbor[frame_nb].clone(),
                );
                match res {
                    Ok((convolutions, target_chunks)) => {
                        assert_eq!(convolutions.len(), target_chunks.len());
